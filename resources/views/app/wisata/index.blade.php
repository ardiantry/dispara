                        ...
                        @forelse($wisatas as $data)
                            <article class="postbox__item format-image mb-60 transition-3">
                                <div class="postbox__thumb w-img mb-30">
                                    <a href="{{ route('app-tour.show', $data['id']) }}">
                                ...
                                <div class="postbox__content">
                                    ...
                                    <h3 class="postbox__title">
                                        <a href="{{ route('app-tour.show', $data['id']) }}">{{ $data['title'] }}</a>
                                    </h3>
                                    <div class="postbox__text">
                                        <p>{{ Str::limit(strip_tags($data['isi']), 330, '...') }}</p>
                                    </div>
                                    <div class="postbox__read-more">
                                        <a href="{{ route('app-tour.show', $data['id']) }}" class="tp-btn">Baca
                                            selengkapnya</a>
                                    </div>
                                </div>
                            </article>
                        @empty
                            <p class="text-center">Oppsss... Tidak ada Destinasi Wisata tersedia</p>
                        @endforelse
                        @if ($wisatas->count())
                                ...
                                <nav>
                                    <ul>
                                        <li>
                                            ...
                                            @else
                                                <a href="{{ $wisatas->previousPageUrl() }}">
                                                    <i class="far fa-angle-left"></i>
                                                </a>
                                            @endif
                                        </li>
                ...
                <div class="col-xxl-4 col-xl-4 col-lg-5 col-md-12">
                    <div class="sidebar__wrapper">
                                        ...
                                        @forelse($recent as $r)
                                            <div class="rc__post mb-20 d-flex align-items-center">
                                                <div class="rc__post-thumb">
                                                    <a href="{{ route('app-tour.show', $r->wisata['id']) }}"><img
                                                            src="{{ asset('storage/' . $r->wisata['image']) }}"
                                                            alt="{{ $r->wisata['title'] }}"
                                                ...
                                                <div class="rc__post-content">
                                                    <h3 class="rc__post-title">
                                                        <a
                                                            href="{{ route('app-tour.show', $r->wisata['id']) }}">{{ $r->wisata['title'] }}</a>
                                                    </h3>
                                                    <div class="rc__meta">
                                                        <span>{{ $r->wisata->created_at->format('M d, Y') }}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        @empty
                                            <p>Kamu belum melakukan kunjungan wisata</p>
                                        @endforelse
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="sidebar__widget mb-40">
                            <h3 class="sidebar__widget-title mb-10">Kategori Wisata</h3>
                            <div class="sidebar__widget-content">
                                <ul>
                                    @foreach ($kategorisWisata as $data)
                                        <li>
                                            <a
                                                href="{{ route('app-tour.index', ['kategori-destinasi' => $data['nama_kategori']]) }}">
                                                {{ $data['nama_kategori'] }}
                                                <span>({{ $data->wisatas_postingan_count }})</span>
                                            </a>
                                        </li>
                                    @endforeach
                                </ul>
