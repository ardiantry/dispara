...
@section('content')
    <div class="row">
        <div class="col">
            <div class="card">
                <div class="card-body">
                    <h4 class="header-title">Tambah Kategori Artikelmu</h4>
                    <p class="card-title-desc">Tambahkan kategori untuk setiap artikelmu agar mempermudah dalam
                        pengelompokkan artikel.</p>
                    <form class="custom-validation" action="{{ route('kategori-artikel.store') }}" method="POST">
                        @csrf
                        <div class="mb-3">
                            <label for="nama_kategori">Kategori Artikel</label>
                            <input type="text" class="form-control" id="nama_kategori" name="nama_kategori" required
                                placeholder="Ketikkan Kategori Artikel" value="{{ old('nama_kategori') }}" autofocus />
                            @error('nama_kategori')
                                <span role="alert">
